...
SVD is a more general decomposition than eigendecomposition.

\begin{theorem}[Existence]
 $\A \in\BR^{m\times n}$ is a nonzero rank $k$ matrix, then $\A\A^T$ is a symmetrical matrix with rank $k$.
 We have eigendecomposition
 \[ \A\A^T \Y = \Y\Lam, \]
 where $\Lam = \text{diag}(\lambda_1, ..., \lambda_k)$ and $\Y^T\Y = \I_k$.
 Let $\Z=\A^T\Y\Lam^{-1/2}$, then 
 \[ \A = \Y\Lam^{1/2}\Z^T, \]
 which is called SVD decomposition.
\end{theorem}

\begin{proof} 
 From the definition of $\Z$, we have $\Z^T\Z = \I_k$ and $\text{span}(\Z) = \text{span}(\A^T)$. 
 Let $\Z_0$ in $\Z$'s complementary space, such that $\hat{\Z}=(\Z, \Z_0)$, $\hat{\Z}^T\hat{\Z} = \hat{\Z}\hat{\Z}^T = \I_n$.
 Then we have
 \[ \A = \A\hat{\Z}\hat{\Z}^T = (\A\Z, \A\Z_0)\hat{\Z}^T = \Y\Lam^{1/2}\Z^T \] 
\end{proof}

\begin{proposition}
 Every matrix $\A \in \BR^{m \times n}$ has a SVD decomposition.
\end{proposition}

Three kinds of SVD decomposition:
\begin{itemize}
 \item $\A = \U\Si\V^T$ is called \textit{full SVD decomposition} if $\A\in\BR^{m \times n}$, $\U\in\BR^{m\times m}$, $\Si\in\BR^{m\times n}$, $\V\in\BR^{n\times n}$.
 \item $\A = \U\Si\V^T$ is called \textit{condensed SVD decomposition} if rank$(A)=k$, $\A\in\BR^{m \times k}$, $\U\in\BR^{m\times k}$, $\Si\in\BR^{k\times k}$, $\V\in\BR^{k\times n}$.
 \item $\A = \U\Si\V^T$ is called \textit{thin SVD decomposition} if $\A\in\BR^{m \times n}$, $\U\in\BR^{m\times n}$, $\Si\in\BR^{n\times n}$, $\V\in\BR^{n\times n}$.
\end{itemize}
